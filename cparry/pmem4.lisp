...
(defun testm () nil) ;;; test_pattern was written in LAP and too long
;; I can't see what is it. anyway ignore it now.
...
(format t "pmem4:input=~a~%" input)
	(setf BUG 10)
	(setf STRUC (CAR INPUT))
    (setf STYPE (CADR INPUT))
    (setf SENT (caddr INPUT))
